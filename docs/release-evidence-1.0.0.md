...
| Frontend quality | TypeScript, Biome on 431 files, 123 frontend test files with 552 tests, the 124-command IPC contract, production build, Storybook build, and Knip passed under the available Node 26.7.0 runtime. The repository pins Node 22.18.0 in `.nvmrc`; `nvm` was not available on this Windows host. | Proven locally on 2026-09-05 |
