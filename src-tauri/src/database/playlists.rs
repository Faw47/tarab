...
impl Database {
    ...
    pub fn playlist_migration_completed(&self, marker: &str) -> SqliteResult<bool> {
        let conn = self.conn.lock();
        Ok(conn
            .query_row(
    ...
    pub fn import_legacy_playlists(
        &self,
        playlists: &[(DbPlaylist, Vec<String>)],
        marker: &str,
    ) -> SqliteResult<()> {
        let mut conn = self.conn.lock();
        let tx = conn.transaction()?;

        for (playlist, track_ids) in playlists {
            tx.execute(
                r#"
                INSERT OR IGNORE INTO playlists (
                  id, name, playlist_type, folder_path, smart_rules, created_at, updated_at,
                  is_pinned, pinned_at, last_synced_at, sync_error
                )
            ...
            let existing_tracks: i64 = tx.query_row(
                "SELECT COUNT(*) FROM playlist_tracks WHERE playlist_id = ?1",
                [&playlist.id],
                |row| row.get(0),
            )?;
            if playlist.playlist_type != "manual" || existing_tracks > 0 {
                continue;
            }
            ...
            let mut stmt = tx.prepare_cached(
                ...
                INSERT OR IGNORE INTO playlist_tracks (
                    playlist_id, track_id, position, added_at, snapshot_title, snapshot_artist,
                    snapshot_album, snapshot_duration, snapshot_file_path, snapshot_has_cover_art,
                    snapshot_cover_art_hash, snapshot_blurhash
                )
                SELECT ?1, ?2, ?3, ?4, t.title, t.artist, t.album, t.duration, t.file_path,
                       COALESCE(t.has_cover_art, 0), t.cover_art_hash, t.blurhash
                FROM (SELECT 1)
        ...
        tx.execute(
            ...
            ON CONFLICT(key) DO UPDATE SET value = 'done', created_at = excluded.created_at,
                expires_at = NULL
            "#,
            params![marker, now_millis_i64_or_default()],
        )?;
        tx.commit()
    }

    pub fn get_playlist_mutation_result(&self, key: &str) -> SqliteResult<Option<String>> {
        let conn = self.conn.lock();
        Ok(conn
            .query_row(
                "SELECT result_json FROM playlist_mutations WHERE mutation_key = ?1",
                [key],
    ...
    pub fn store_playlist_mutation_result(&self, key: &str, result: &str) -> SqliteResult<()> {
        let conn = self.conn.lock();
        conn.execute(
            "UPDATE playlist_mutations SET result_json = ?2 WHERE mutation_key = ?1",
            params![key, result],
        )?;
        Ok(())
    }
    ...
    pub fn set_playlist_tracks(
        ...
        self.set_playlist_tracks_inner(playlist_id, track_ids, updated_at, None)
            .map(|_| ())
    }

    pub fn set_playlist_tracks_idempotent(
        &self,
        playlist_id: &str,
        track_ids: &[String],
        updated_at: i64,
        ...
        self.set_playlist_tracks_inner(playlist_id, track_ids, updated_at, Some(mutation_key))
    }

    fn set_playlist_tracks_inner(
        &self,
        playlist_id: &str,
        track_ids: &[String],
        updated_at: i64,
        ...
        if let Some(key) = mutation_key {
            let inserted = tx.execute(
                "INSERT OR IGNORE INTO playlist_mutations (mutation_key, result_json, created_at) VALUES (?1, NULL, ?2)",
    ...
    pub fn add_tracks_to_playlist(
        ...
        self.add_tracks_to_playlist_inner(playlist_id, track_ids, None)
            .map(|_| ())
    }

    pub fn add_tracks_to_playlist_idempotent(
        &self,
        playlist_id: &str,
        track_ids: &[String],
        mutation_key: &str,
    ) -> SqliteResult<bool> {
        self.add_tracks_to_playlist_inner(playlist_id, track_ids, Some(mutation_key))
    }

    fn add_tracks_to_playlist_inner(
        &self,
        playlist_id: &str,
        track_ids: &[String],
        mutation_key: Option<&str>,
