...
use crate::desktop_integration::{
    replace_menu_handles, request_desktop_control_action, DesktopControlAction,
};

#[cfg(target_os = "macos")]
pub fn build_menu(app: &App<Wry>) -> tauri::Result<()> {
    ...
    let play_pause = MenuItemBuilder::with_id("playback.toggle", "Play").build(app)?;
    let previous = MenuItemBuilder::with_id("playback.previous", "Previous")
        .accelerator("Cmd+Left")
        .build(app)?;
    let next = MenuItemBuilder::with_id("playback.next", "Next")
    ...
    app.set_menu(menu)?;
    replace_menu_handles(app.handle(), play_pause, previous, next);

    app.on_menu_event(move |app, event| {
        let id = event.id().as_ref();
        let control = match id {
            "quit" => Some(DesktopControlAction::Quit),
            "playback.toggle" => Some(DesktopControlAction::TogglePlay),
            "playback.previous" => Some(DesktopControlAction::Previous),
            "playback.next" => Some(DesktopControlAction::Next),
            "playback.seek-backward" => Some(DesktopControlAction::SeekBackward),
            "playback.seek-forward" => Some(DesktopControlAction::SeekForward),
            "playback.shuffle" => Some(DesktopControlAction::ToggleShuffle),
            "playback.repeat" => Some(DesktopControlAction::CycleRepeat),
            _ => None,
        };
        if let Some(action) = control {
            request_desktop_control_action(app, action);
        } else {
