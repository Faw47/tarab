...
impl<R: Runtime> Media<R> {
    fn controller(
        &self,
    ) -> crate::Result<MutexGuard<'_, Box<dyn platform::MediaController + Send>>> {
        self.controller
            .lock()
            .map_err(|_| crate::Error::String("media controller lock poisoned".to_string()))
    }
        ...
        F: Fn(MediaControlEvent) + Send + 'static,
    {
        match self.controller() {
            Ok(mut controller) => controller.set_event_handler(Box::new(handler)),
            Err(err) => eprintln!("failed to set media event handler: {}", err),
        }
    }
