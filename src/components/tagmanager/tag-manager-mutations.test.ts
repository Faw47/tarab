...
describe('tag manager mutation helpers', () => {
  ...
  it('preserves blank extra-tag fields but deletes explicitly removed or renamed keys', () => {
    expect(
      buildExtraTagUpdates({ MOOD: 'Calm', LABEL: 'Indie' }, [
        { key: 'MOOD', value: '' },
        { key: 'PUBLISHER', value: 'Tarab' },
      ]),
    ).toEqual({ PUBLISHER: 'Tarab', LABEL: '' });
  });
});
