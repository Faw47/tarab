...
].join(' '),
  {
    variants: {
      variant: {
        ...
        outline: 'text-text-primary border border-[var(--adl-liquid-border)] hover:border-[var(--adl-liquid-border-hover)]',
...
function getMergedLiquidVars(
  ...
  const baseHighlight = 'inset 0 1px 1px rgba(255, 255, 255, 0.2), inset 0 -1px 1px rgba(0, 0, 0, 0.05)';
  ...
  let vars: Record<string, string> = {
    ...
    '--adl-liquid-bg': 'linear-gradient(180deg, rgba(255, 255, 255, 0.15) 0%, rgba(255, 255, 255, 0.09) 40%, rgba(255, 255, 255, 0.018) 100%)',
    '--adl-liquid-bg-hover': 'linear-gradient(180deg, rgba(255, 255, 255, 0.21) 0%, rgba(255, 255, 255, 0.13) 42%, rgba(255, 255, 255, 0.04) 100%)',
    '--adl-liquid-bg-active': 'linear-gradient(180deg, rgba(255, 255, 255, 0.07) 0%, rgba(255, 255, 255, 0.02) 100%)',
  ...
  if (safeVariant === 'primary') {
    ...
    vars = {
      ...vars,
      '--adl-liquid-bg': 'linear-gradient(180deg, rgba(255, 59, 48, 0.26) 0%, rgba(255, 59, 48, 0.14) 40%, rgba(255, 59, 48, 0.045) 100%)',
      '--adl-liquid-bg-hover': 'linear-gradient(180deg, rgba(255, 59, 48, 0.34) 0%, rgba(255, 59, 48, 0.20) 42%, rgba(255, 59, 48, 0.09) 100%)',
      '--adl-liquid-bg-active': 'linear-gradient(180deg, rgba(255, 59, 48, 0.14) 0%, rgba(255, 59, 48, 0.03) 100%)',
      '--adl-liquid-text': 'rgba(255, 236, 236, 0.98)',
      '--adl-liquid-highlight': 'inset 0 1px 1px rgba(255, 160, 160, 0.60), inset 0 -1px 1px rgba(0, 0, 0, 0.1)',
  ...
  (
    ...
    const handlePointerDown = React.useCallback(
      (event: React.PointerEvent<HTMLButtonElement>) => {
        ...
        onPointerDown?.(event);
      }, [disabled, finalReducedEffects, bypassEffects, measure, onPointerDown, scheduleUpdate, trackerEnabled],
    );
    ...
    const mergedStyle = React.useMemo(() => {
      ...
      const mergedTransform = isNeobrutalism ? baseTransform : composeTransform(baseTransform, targetScale);
      ...
      return {
        ...
        color: isNeobrutalism ? style?.color : (style?.color ?? toneVarsRecord['--adl-liquid-text']),
        ...
        willChange: !finalReducedEffects && !isNeobrutalism && (hovered || pressed) && !useInsetPress ? 'transform' : 'auto',
        boxShadow,
      } as React.CSSProperties;
    }, [finalReducedEffects, hovered, isNeobrutalism, pressed, style, targetScale, toneVars, useInsetPress]);
